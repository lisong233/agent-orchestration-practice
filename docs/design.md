...
| Web | Gradio 6 | 快速原型，满足评委 5 步流程，节点小宝内网穿透 |
...
`audit_log.py` 将每次裁决的完整中间过程（label / verdicts / evidence / reason / form_notes / revision_count / timestamp）写入 `logs/runs.jsonl`。隐藏测试集不给参赛者→日志是唯一复盘数据源。文档解析支持 .docx/.doc 三级降级（doc-read → LibreOffice → python-docx → 纯文本）。
