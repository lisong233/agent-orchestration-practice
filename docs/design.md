...
`bash docs/setup.sh`（自动：创建 venv → 安装依赖 → 提示填入 `DEEPSEEK_API_KEY` → 启动 Gradio Web + `share=True` 公网隧道）。Windows 用 `docs/setup.ps1`。
...
| 2 | **Gradio URL 有效期 ~1 周**：容器重启后 URL 变更 | 备选 Cloudflare Tunnel / ngrok / HF Spaces |
