...
```
本地 (Windows) ──SCP──→ NAS (UGOS) ──docker compose──→ 容器 (python:3.12-slim)
                              │
...
```bash
# 用 SSH pipe 绕过 scp 权限问题（NAS 上 scp dest open 偶发失败）
cat src/aiarmy/web.py | ssh nas-wan "cat > /volume1/docker/aiarmy/src/aiarmy/web.py"
cat src/aiarmy/graph.py | ssh nas-wan "cat > /volume1/docker/aiarmy/src/aiarmy/graph.py"
```
...
curl -s http://100.66.1.1:7860 | grep "并发 3"

# 检查公网 URL
ssh nas-wan "docker logs aiarmy 2>&1 | grep gradio.live"
```
...
**为什么自动下载失败**：NAS 容器直连 huggingface.co 可能超时或被墙。
...
# 2. 传到 NAS 并注入容器
cat /tmp/frpc_linux_amd64_v0.3 | ssh nas-wan \
  "cat > /tmp/frpc_v0.3 && \
...
# 3. 重启容器触发 share
ssh nas-wan "docker restart aiarmy"
```

**根治方案（TODO）**：在 Dockerfile 中预置 frpc 二进制，避免每次 rebuild 后手动注入。

### 问题 2：scp 到 NAS 偶发 `No such file or directory`

即使 `ssh nas-wan "ls /path/"` 确认目录存在，scp 仍可能报 `dest open: No such file or directory`。
...
| NAS 路径 | `/volume1/docker/aiarmy/` |
| SSH 入口 | `ssh nas-wan`（节点小宝 100.66.1.1） |
